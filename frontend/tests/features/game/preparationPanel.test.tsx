...
describe('preparation progression', () => {
  it('enforces three objectives and lets the player deselect one', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Set Meeting Objectives →' }))
    const firstObjective = screen.getByRole('button', { name: /Create a reliable/ })
    fireEvent.click(firstObjective)
    fireEvent.click(screen.getByRole('button', { name: /Replace Meridian's entire/ }))
    fireEvent.click(screen.getByRole('button', { name: /Build more dashboards/ }))
    expect(screen.getByText('3 / 3 selected')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Use AI to predict/ })).toBeDisabled()
    fireEvent.click(firstObjective)
    expect(screen.getByRole('button', { name: /Use AI to predict/ })).not.toBeDisabled()
  })
