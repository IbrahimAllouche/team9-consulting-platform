...
describe('completion arrival', () => {
  it.each([1, 2, 3])('keeps Level %i celebration through its animation and unlocks the next room', (level) => {
    vi.useFakeTimers()
    ...
    render(<ConsultingRoom stage={stage} />)
    expect(screen.getByText('Celebration visible')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: `ENTER LEVEL ${level + 1}` })).toHaveAttribute('href', stage.href)
